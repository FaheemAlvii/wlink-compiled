from odoo import fields, api, models  # noqa
...
class SelectConnection(models.TransientModel):
    ...
    def get_selected_connection(self):
        ...
        connection = self.env['whatsapp.connection'].search([('id', '=', connection_id)], limit=1)  # noqa
        return connection

    @api.model
    def default_get(self, fields):
        result = super(SelectConnection, self).default_get(fields)  # noqa
    ...
    def onchange_connection(self):
        if self.connection:
            Config().set('pos_connection', self.connection.id)